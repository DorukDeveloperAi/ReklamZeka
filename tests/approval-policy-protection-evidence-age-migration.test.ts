...
const migration = readFileSync(
  new URL("../drizzle/20260807221641_perpetual_black_widow.sql", import.meta.url), "utf8",
);

describe("ApprovalPolicy protection evidence age migration boundary", () => {
  ...
  it("requires the explicit JSON number 1..604800 in definitions and queue snapshots", () => {
    expect(migration.match(/\? 'maximumProtectionEvidenceAgeSeconds'/g)).toHaveLength(2);
    expect(migration.match(/maximumProtectionEvidenceAgeSeconds}'\) = 'number'/g)).toHaveLength(2);
    expect(migration.match(/maximumProtectionEvidenceAgeSeconds}'\)::integer between 1 and 604800/g)).toHaveLength(2);
