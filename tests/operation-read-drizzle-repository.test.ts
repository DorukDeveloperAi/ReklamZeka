...
const row = (name: string, ref: string, kind = 0, aref = "") => ({ campaign_id: "33333333-3333-4333-8333-333333333333", external_campaign_id: "meta_campaign_1", campaign_name: name, campaign_ref: ref, account_id: "22222222-2222-4222-8222-222222222222", account_name: "Account", adset_id: kind ? "44444444-4444-4444-8444-444444444444" : null, external_ad_set_id: kind ? "meta_adset_1" : null, adset_name: kind ? "Set" : null, cbo: true, org_id: null, org_name: null, market: "yerli", days: ["2026-08-17"], spend: 33, spend_count: 1, currencies: 1, campaign_budget_out: kind ? null : 1000, adset_budget_out: null, kind, aref });
...
describe("DrizzleOperationReadRepository", () => {
  ...
  const scopeRows = (hash = revision().definitionHash, candidates: unknown[] = [{ entity_level: "campaign", entity_id: ids.campaign, campaign_id: ids.campaign, market_value_id: null, market_dimension_id: null, market_key: null }]) => [
    [{ slice_id: "99999999-9999-4999-8999-999999999999", revision_id: ids.revision, slice_ref: "slice_live", revision_ref: "slice_revision_live", revision_number: 2, definition_hash: hash, market_dimension_id: "11111111-1111-4111-8111-111111111111", market_dimension_key: "market", market_value_id: "22222222-2222-4222-8222-222222222222", market_key: "yerli" }],
  ...
  it("uses read-only RR, canonical metric aggregation, and opaque limit+1 cursor", async () => {
    ...
    expect(result.facts[0]).toMatchObject({ spendMinor: null, observedDays: ["2026-08-17"] });
    expect(result.nextCursor).toMatch(/^operation_cursor_/);
    // The visible page uses a separate bounded primary-result phase after the
    // canonical facts query; it cannot alter pagination or fan out metrics.
  ...
  it("keeps campaign summary and ad-set siblings in the cursor order", async () => {
    ...
    expect(cursor).toMatchObject({ v: 3, k: 1, a: "ad_set_a" });
  });
