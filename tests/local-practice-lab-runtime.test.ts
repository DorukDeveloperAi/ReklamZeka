...
describe("local Practice Lab route", () => {
  it("reuses the canonical local principal and returns a real empty tenant read", async () => {
    ...
    expect(claims.scopes).toEqual(["approval_queue:decide", "approval_queue:read", "budget_lab:draft", "budget_lab:read", "decision_room:mark_read", "decision_room:read", "practice_lab:read", "promotion_catalog:read", "promotion_preflight:read"]);
