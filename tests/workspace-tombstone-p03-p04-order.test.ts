...
describe("P03/P04 workspace tombstone inventory and dependency order",()=>{
  it("inspects every immutable Guide and Slice table",()=>{
    for(const table of ["guide_activation_outbox","guide_lifecycle_events","guide_interpretation_acceptances","guide_revision_actions","guide_revision_budget_refs","guide_heads","guide_revisions","guides","slice_resolution_snapshot_members","slice_resolution_snapshots","slice_revision_predicate_values","slice_revision_overrides","slice_revision_predicates","slice_revisions","slices"]) expect(WORKSPACE_TOMBSTONE_PURGE_TABLES).toContain(table);
  });
