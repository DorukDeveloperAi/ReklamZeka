...
describe("operation read model", () => {
  ...
  it("keeps a bound zero result and rejects untrusted fact fields", () => {
    const model = buildOperationReadModel({ workspaceId: ws, period: operationPeriod({ kind: "today", now: new Date("2026-08-17T12:00:00Z") }), facts: [fact({ observedDays: ["2026-08-17"], missingDays: [], primaryResultBinding: { state: "bound", actionType: "lead", bindingRef: "metric_lead" }, primaryResult: "0", primaryResultCostMinor: null, primaryResultSource: "slice_binding" })] });
    expect(model.rows[0]).toMatchObject({ primaryResultState: "bound", primaryResult: "0", primaryResultCostMinor: null });
