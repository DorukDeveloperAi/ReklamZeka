...
import { OperationPaginationGate, OperationTableSurface, applyOperationSavedView, operationAppendRows, operationHierarchy, operationProjectionReadState, operationViewSummary, parseOperationSavedView, parseOperationTableProjection } from "@/app/dashboard/operation-table-panel";

const row = Object.freeze({
  market: "yerli",
  accountRef: "account_1",
  organizationCampaignRef: null,
  organizationCampaignName: "Kurum Kampanyası atanmadı",
  campaignRef: "campaign_1",
  adSetRef: null,
  accountName: "Ana hesap",
  campaignName: "Kampanya A",
  adSetName: null,
  currentBudgetMinor: 12000,
  budgetOwner: "campaign",
  budgetOwnerRef: "campaign_1",
  spendMinor: 3400,
  primaryResultState: "unbound",
  primaryResult: null,
  primaryResultCostMinor: null,
  primaryResultSource: "unbound",
  sourceState: "partial",
  missingDays: ["2026-08-17"],
  reasonCodes: ["insight_missing"],
});
const projection = Object.freeze({
  version: "operation-read/2.0.0",
  period: { startDate: "2026-08-11", endDate: "2026-08-17" },
  state: "partial",
  rows: [
    row,
    {
      ...
      adSetName: "Set A",
      budgetOwner: "ad_set",
      budgetOwnerRef: "ad_set_1",
    },
  ],
  budgetOwners: [{ ref: "campaign_1", currentBudgetMinor: 12000 }],
  authority: { canWriteMeta: false, canExecute: false, canApprove: false },
  nextCursor: "operation_cursor_next",
});
...
describe("operation table panel", () => {
  it("accepts only the closed canonical read contract", () => {
    ...
    expect(
      parseOperationTableProjection({
        ...projection,
        authority: { canWriteMeta: true, canExecute: false, canApprove: false },
      }),
    ).toBeNull();
    expect(
      parseOperationTableProjection({
        ...projection,
        rows: [{ ...row, primaryResult: 3 }],
      }),
    ).toBeNull();
    expect(
      parseOperationTableProjection({
        ...projection,
        period: { startDate: "2026-02-31", endDate: "2026-03-01" },
      }),
    ).toBeNull();
    expect(
      parseOperationTableProjection({
        ...projection,
        period: { startDate: "2026-08-18", endDate: "2026-08-17" },
      }),
    ).toBeNull();
  });
  ...
  it("keeps the canonical campaign summary above its ad-set child", () => {
    ...
    expect([...hierarchy[0]!.campaigns.values()][0]).toMatchObject({
      summary: { campaignName: "Kampanya A" },
      adSets: [{ adSetName: "Set A" }],
    });
  });

  it("applies a bounded saved view without inventing monetary subtotals", () => {
    const view = parseOperationSavedView({
    ...
    const visible = applyOperationSavedView(parseOperationTableProjection(projection)!.rows, view!);
    expect(visible).toHaveLength(1);
    expect(visible[0]).toMatchObject({ adSetRef: "ad_set_1" });
    expect(operationViewSummary(visible)).toEqual({
      rows: 1,
      ready: 0,
      incomplete: 1,
      ratioAvailable: 0,
    });
    expect(parseOperationSavedView({ ...view, search: "x".repeat(81) })).toBeNull();
    expect(parseOperationSavedView({ ...view, unknown: true })).toBeNull();
  });

  it("renders source gaps, unbound primary result and responsive card fallback without demo data", () => {
    const html = renderToStaticMarkup(
      createElement(OperationTableSurface, {
        state: {
          status: "ready",
          projection: parseOperationTableProjection(projection)!,
        },
        period: "7d",
        onPeriod: vi.fn(),
        onLoadMore: vi.fn(),
    ...
    expect(html).toContain("Görünümü kaydet");
    expect(html).toContain("Filtrelenmiş görünüm özeti");
    expect(html).toContain("Satır kanıtını incele");
    ...
    expect(source).toMatch(/\.mobileCards\s*\{[\s\S]*display:\s*grid/);
    expect(source).toMatch(/\.card\s*\{[\s\S]*display:\s*grid;[\s\S]*min-width:\s*0/);
    expect(source).toMatch(/\.card h3,[\s\S]*\.card dd\s*\{[\s\S]*min-width:\s*0;[\s\S]*overflow-wrap:\s*anywhere/);
  ...
  it("keeps ad-set-only scopes as ad-set facts and uses stable canonical hierarchy keys", () => {
    const adSetOnly = parseOperationTableProjection({
      ...projection,
      rows: [
        {
          ...row,
          adSetRef: "ad_set_only",
          adSetName: "Tek Set",
          budgetOwner: "ad_set",
          budgetOwnerRef: "ad_set_only",
        },
      ],
    })!;
    const campaign = [...operationHierarchy(adSetOnly.rows)[0]!.campaigns.values()][0]!;
    expect(campaign.summary).toBeNull();
    expect(campaign.adSets[0]).toMatchObject({
      adSetName: "Tek Set",
      campaignName: "Kampanya A",
    });
    const html = renderToStaticMarkup(
      createElement(OperationTableSurface, {
        state: { status: "ready", projection: adSetOnly },
        period: "7d",
        onPeriod: vi.fn(),
        onLoadMore: vi.fn(),
        onConnect: vi.fn(async () => true),
    ...
    const duplicateLabels = operationHierarchy([
      {
        ...adSetOnly.rows[0]!,
        accountRef: "account_2",
        campaignRef: "campaign_2",
      },
      {
        ...adSetOnly.rows[0]!,
        accountRef: "account_3",
        campaignRef: "campaign_3",
      },
    ]);
  ...
  it("renders a canonical unavailable projection as an unavailable source, never a ready table", () => {
    const unavailable = parseOperationTableProjection({
      ...projection,
      state: "unavailable",
      rows: [],
      budgetOwners: [],
