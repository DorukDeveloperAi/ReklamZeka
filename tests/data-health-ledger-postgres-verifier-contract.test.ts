...
describe("P01-E outer rollback verifier contract", () => {
  it("uses an explicit post-apply boundary and outer rollback verification", () => {
    for (const token of ["migration_not_applied", 'client.query("begin")', 'client.query("rollback")', "zeroResidue", "tamperRejected", "crossTenantRejected", "concurrentLostHeadPrevented"]) expect(verifier).toContain(token);
  });
});
