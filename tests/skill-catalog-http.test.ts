...
const service = { list: vi.fn(async () => ({ contractVersion: "skill-catalog-ui/1.0.0", sources: [{ optionId, title: "Meta yardım", url: "https://www.facebook.com/business/help/learning", freshness: "fresh" }], playbooks: [{ kind: "playbook", ref: "playbook_alpha", revision: 1, state: "active", title: "T", body: "private snippet" }] })),
...
describe("skill catalog http boundary", () => {
  it("reads only through same-origin cookie session and strips snippet bodies from the Agent projection", async () => {
    ...
    expect(JSON.stringify(agent)).not.toContain(optionId);
  ...
  it("allows only explicit user create/revise intents and requires revision confirmation without opening action authority", async () => {
    const handlers = createSkillCatalogHttpHandlers({ service, resolve: async () => principal });
    const create = await handlers.POST(request("POST", "skill-playbook-create", { title: "t", body: "b", sourceOptionId: optionId }));
    ...
    expect((await handlers.POST(request("POST", "skill-playbook-revise", { playbookRef: "playbook_alpha", expectedRevision: 1, title: "t", body: "b", sourceOptionId: optionId }))).status).toBe(400);
    expect((await handlers.POST(request("POST", "skill-playbook-revise", { playbookRef: "playbook_alpha", expectedRevision: 1, title: "t", body: "b", sourceOptionId: optionId, confirmed: true }))).status).toBe(201);
    expect((await handlers.POST(request("POST", "skill-playbook-create", { title: "t", body: "b", sourceRef: "source_x" }))).status).toBe(400);
