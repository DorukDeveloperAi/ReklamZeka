...
describe("organization campaign postgres verifier", () => it("is bounded and DNS-explicit", () => {
  ...
  expect(source).toContain("external_dns_unavailable"); expect(source).toContain("organization_campaign_migration_not_applied"); expect(source).toContain("organization_campaign_temporal_assertion_failed"); expect(source).toContain("rollback");
}));
