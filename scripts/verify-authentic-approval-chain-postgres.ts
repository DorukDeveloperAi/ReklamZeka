...
finally { globalThis.fetch = fetch0; if (fixture) { const residual = rows(await database.execute(sql`select count(*)::int as count from budget_proposal_versions where workspace_id=${fixture.workspaceId}::uuid`))[0]; evidence.rollbackClean = Number(residual?.count) === 0; const purge = new DrizzleWorkspaceTombstonePurgePort(); let cleanupPool = createPool(); let cleanupDatabase = drizzle(cleanupPool, { schema }); try { for (const workspaceId of [fixture.workspaceId, fixture.foreignWorkspaceId]) { for (let attempt = 0; ; attempt += 1) { try { await retryWorkspaceTombstoneTransport({ execute: async () => { const tombstones = new WorkspaceTombstoneService(new DrizzleWorkspaceTombstoneStore(cleanupDatabase as never, purge), { authorize: async (input) => input.approvalRef === "ephemeral-fixture-approved" }, fixture!.actorId, 60_000); const plan = await tombstones.dryRun(workspaceId, new Date().toISOString()); await tombstones.execute({ planRef: plan.planRef, approvalRef: "ephemeral-fixture-approved", now: new Date().toISOString() }); }, reconnect: async () => { await cleanupPool.end(); cleanupPool = createPool(); cleanupDatabase = drizzle(cleanupPool, { schema }); }, completedAfterReconnect: async () => rows(await cleanupDatabase.execute(sql`select lifecycle_state from workspaces where id=${workspaceId}::uuid`))[0]?.lifecycle_state === "tombstoned" && (await purge.inspect(cleanupDatabase as never, workspaceId)).candidateCount === 0 }); break; } catch (error) { if (!serializationFailure(error) || attempt === 2) throw error; } } } evidence.cleanup = (await purge.inspect(cleanupDatabase as never, fixture.workspaceId)).candidateCount === 0 && (await purge.inspect(cleanupDatabase as never, fixture.foreignWorkspaceId)).candidateCount === 0; } finally { await cleanupPool.end(); } } await pool.end(); }
