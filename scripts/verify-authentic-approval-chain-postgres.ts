...
try {
  ...
  await database.transaction(async (tx) => {
    ...
    evidence.selected = (await select.append({ workspaceId: source.workspaceId, draftHash: savedRule.draft.draftHash, proposalHash: saved.proposal.proposalHash, scenarioRef: "scenario.decrease", allocationRef: "allocation_primary", idempotencyKey: "selection.approval_chain.r1", selectedAt: proposedAt, actorId: source.actorId })).outcome === "inserted";
...
finally { globalThis.fetch = fetch0; if (fixture) { const residual = rows(await database.execute(sql`select count(*)::int as count from budget_proposal_versions where workspace_id=${fixture.workspaceId}::uuid`))[0]; evidence.rollbackClean = Number(residual?.count) === 0; await database.execute(sql`delete from workspaces where id in (${fixture.workspaceId}::uuid, ${fixture.foreignWorkspaceId}::uuid)`); evidence.cleanup = rows(await database.execute(sql`select count(*)::int as count from workspaces where id in (${fixture.workspaceId}::uuid, ${fixture.foreignWorkspaceId}::uuid)`))[0]?.count === 0; } await pool.end(); }
if (!Object.values(evidence).every((v) => v === true) || evidence.metaCalls !== 0 || evidence.executionCalls !== 0) throw new Error(`authentic_approval_chain_failed:${JSON.stringify(evidence)}`);
