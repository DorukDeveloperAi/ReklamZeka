...
let repositoryReplay = false, singleClientCas = false, fencingAndReclaim = false, directLifecycleProbes = false, canonicalJsonVectors = false, immutableArtifact = false, schedulerReceiptReplay = false, crashResume = false, p01LedgerReplay = false, humanTriagePreserved = false, tamperRejected = false, forgedArtifactRejected = false, tenantFkRejected = false, tombstonePurge = false;
...
try {
  ...
  try {
    ...
    appendOnlyTriggers = Number(triggers.rows[0]?.n ?? 0) === 6;
