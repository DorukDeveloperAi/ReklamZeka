...
try { const database=drizzle(pool,{schema}); const today=new Date(); const start=new Date(today.valueOf()-7*86400_000); const service=createDrizzleProductionMetaReadSyncService({database,scopeResolver:{resolve:async()=>({workspaceId,connectionId:"6d695103-4dc0-44ba-8a1b-67702449c4a1"})},environment:process.env,deferAffectedGeoMaterialization:true,inventoryTransactionMode:"idempotent_page",durableTransactionMode:"idempotent_checkpoint"}); const result=await service.run({parentRunId:`meta.read.initial.${today.toISOString().slice(0,10).replaceAll("-","")}`,dateStart:start.toISOString().slice(0,10),dateStop:today.toISOString().slice(0,10),dateSliceDays:7,initialPageSize:10,requestTimeoutMs:20_000,maxAttempts:3}); console.log(JSON.stringify(result)); } finally {await pool.end();}
