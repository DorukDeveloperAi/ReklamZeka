...
export async function materializeCurrentEffectiveAnalysisContextSourceFixture(database: Database, now = new Date()): Promise<Readonly<{
  ...
  /** Reviewed guidance identity returned by the normal current-reader projection. */
  reviewedGuidanceSet: Readonly<{ setRef: string; setVersion: number; setHash: string }>;
  /** Immutable policy-authority basis returned by its normal materializer. */
  authoritySnapshot: Readonly<{ snapshotRef: string; snapshotHash: string; catalogHash: string }>;
}>> {
