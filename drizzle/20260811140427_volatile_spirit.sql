...
--> statement-breakpoint
-- The tenant-composite key must exist before heads/invalidations add scoped FKs.
CREATE UNIQUE INDEX "candidate_preview_binding_revisions_workspace_row_unique" ON "candidate_preview_binding_revisions" USING btree ("workspace_id","id");
--> statement-breakpoint
-- This pre-existing parent also needs its tenant-composite key before the scoped FK below.
CREATE UNIQUE INDEX "guidance_sets_workspace_row_unique" ON "guidance_sets" USING btree ("workspace_id","id");
--> statement-breakpoint
