...
-- Agent producer invariant: category='data' and state='proposed' and event_type='proposed';
-- system may append observation events without changing a human triage state.
CREATE TABLE finding_lifecycle_events (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE cascade,
 namespace text NOT NULL, resolution_scope text NOT NULL, scope_ref text NOT NULL, fingerprint text NOT NULL, sequence integer NOT NULL,
 ...
 CONSTRAINT finding_lifecycle_events_contract CHECK (
  namespace ~ '^[a-z][a-z0-9_.:-]{0,63}$' AND octet_length(resolution_scope) BETWEEN 1 AND 256 AND scope_ref ~ '^(workspace|account)_[a-f0-9]{24}$' AND fingerprint ~ '^[a-z][a-z0-9_.:-]{0,127}$'
...
CREATE TABLE finding_heads (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE cascade,
 namespace text NOT NULL, resolution_scope text NOT NULL, scope_ref text NOT NULL, fingerprint text NOT NULL, sequence integer NOT NULL, state text NOT NULL,
 evidence_hash text NOT NULL, event_hash text NOT NULL, updated_at timestamptz NOT NULL DEFAULT now(),
 CONSTRAINT finding_heads_contract CHECK (namespace ~ '^[a-z][a-z0-9_.:-]{0,63}$' AND octet_length(resolution_scope) BETWEEN 1 AND 256 AND scope_ref ~ '^(workspace|account)_[a-f0-9]{24}$' AND fingerprint ~ '^[a-z][a-z0-9_.:-]{0,127}$' AND sequence BETWEEN 1 AND 1000000 AND state IN ('open','resolved') AND evidence_hash ~ '^[a-f0-9]{64}$' AND event_hash ~ '^[a-f0-9]{64}$')
);
...
CREATE INDEX finding_lifecycle_events_workspace_scope_time_idx ON finding_lifecycle_events(workspace_id,namespace,resolution_scope,occurred_at);
CREATE INDEX finding_lifecycle_events_workspace_eligible_scope_idx ON finding_lifecycle_events(workspace_id,namespace,resolution_scope,scope_ref);
...
CREATE INDEX finding_heads_workspace_eligible_scope_idx ON finding_heads(workspace_id,namespace,resolution_scope,scope_ref);
...
CREATE INDEX development_log_events_workspace_scope_time_idx ON development_log_events(workspace_id,namespace,resolution_scope,occurred_at);
...
CREATE INDEX development_log_events_workspace_actor_fk_idx ON development_log_events(workspace_id,actor_user_id);
...
CREATE OR REPLACE FUNCTION public.finding_head_exact_advance_guard() RETURNS trigger LANGUAGE plpgsql SECURITY INVOKER SET search_path='' AS $$ DECLARE e public.finding_lifecycle_events%ROWTYPE; BEGIN IF TG_OP='DELETE' THEN IF EXISTS(SELECT 1 FROM public.workspaces WHERE id=OLD.workspace_id AND lifecycle_state='tombstoning') THEN RETURN OLD; END IF; RAISE EXCEPTION 'finding heads may only be deleted by workspace tombstone'; END IF; SELECT * INTO e FROM public.finding_lifecycle_events WHERE workspace_id=NEW.workspace_id AND event_hash=NEW.event_hash; IF NOT FOUND OR (e.namespace,e.resolution_scope,e.scope_ref,e.fingerprint,e.sequence,e.state,e.evidence_hash)<>(NEW.namespace,NEW.resolution_scope,NEW.scope_ref,NEW.fingerprint,NEW.sequence,NEW.state,NEW.evidence_hash) THEN RAISE EXCEPTION 'finding head must exactly reference event'; END IF; IF TG_OP='INSERT' THEN IF NEW.sequence<>1 OR e.previous_event_hash<>repeat('0',64) OR e.event_type<>'opened' OR e.state<>'open' THEN RAISE EXCEPTION 'finding head must begin opened at genesis'; END IF; ELSE IF (NEW.workspace_id,NEW.namespace,NEW.resolution_scope,NEW.scope_ref,NEW.fingerprint)<>(OLD.workspace_id,OLD.namespace,OLD.resolution_scope,OLD.scope_ref,OLD.fingerprint) OR NEW.sequence<>OLD.sequence+1 OR e.previous_event_hash<>OLD.event_hash OR NOT ((OLD.state='open' AND e.event_type='observed' AND e.state='open') OR (OLD.state='open' AND e.event_type='resolved' AND e.state='resolved') OR (OLD.state='resolved' AND e.event_type='reopened' AND e.state='open')) THEN RAISE EXCEPTION 'finding head requires exact next lifecycle transition'; END IF; END IF; RETURN NEW; END; $$;
