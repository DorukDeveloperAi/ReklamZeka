-- Forward repair for the already-applied canonical registry migration: DELETE
-- has no NEW row, so purge authorization must be decided before NEW fields.
...
BEGIN
  ...
  IF OLD.tombstoned_at IS NULL AND NEW.tombstoned_at IS NOT NULL
    AND NEW.id = OLD.id AND NEW.workspace_id = OLD.workspace_id AND NEW.slice_ref = OLD.slice_ref AND NEW.label = OLD.label
  ...
  IF NEW.id = OLD.id AND NEW.workspace_id = OLD.workspace_id AND NEW.slice_ref = OLD.slice_ref AND NEW.label = OLD.label
    AND NEW.market_definition_id = OLD.market_definition_id AND NEW.created_by_actor_id = OLD.created_by_actor_id
