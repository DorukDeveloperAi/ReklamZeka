...
BEGIN
  ...
  IF (SELECT count(*) FROM jsonb_object_keys(p))<>14 OR NOT(p ?& ARRAY['version','workspaceRef','accountRef','templateRef','revision','previousRevisionHash','state','namingFamily','entityLevel','nameRules','corroboration','proposedAssignments','authority','revisionHash'])
    ...
    OR p#>>'{authority,canPropose}'<>'true' OR p#>>'{authority,canAssign}'<>'false' OR p#>>'{authority,canPublish}'<>'false' OR p#>>'{authority,canApprove}'<>'false' OR p#>>'{authority,canExecute}'<>'false' OR p#>>'{authority,canWriteMeta}'<>'false'
  ...
  FOR rule IN SELECT value FROM jsonb_array_elements(p->'nameRules') LOOP
    IF jsonb_typeof(rule)<>'object' OR (SELECT count(*) FROM jsonb_object_keys(rule))<>3 OR NOT(rule ?& ARRAY['source','match','tokens']) OR rule->>'source' NOT IN('campaign_name','ad_set_name') OR rule->>'match' NOT IN('all','any') OR NEW.entity_level='campaign' AND rule->>'source'='ad_set_name' OR jsonb_typeof(rule->'tokens')<>'array' OR jsonb_array_length(rule->'tokens') NOT BETWEEN 1 AND 12 OR EXISTS(SELECT 1 FROM jsonb_array_elements_text(rule->'tokens') token WHERE token!~'^[[:alnum:]]{1,64}$') OR (SELECT count(*) FROM jsonb_array_elements_text(rule->'tokens'))<>(SELECT count(DISTINCT token) FROM jsonb_array_elements_text(rule->'tokens') token) THEN RAISE EXCEPTION 'naming rule invalid'; END IF;
  END LOOP;
  FOR selector IN SELECT value FROM jsonb_array_elements(p->'corroboration') LOOP
    IF jsonb_typeof(selector)<>'object' OR (SELECT count(*) FROM jsonb_object_keys(selector))<>3 OR NOT(selector ?& ARRAY['kind','operator','expected']) OR selector->>'kind' NOT IN('objective','optimization','geo','targeting','platform','creative','cta','destination') OR selector->>'operator' NOT IN('equals','includes_all','includes_any','present') OR jsonb_typeof(selector->'expected')<>'array' OR selector->>'operator'='present' AND jsonb_array_length(selector->'expected')<>0 OR selector->>'operator'<>'present' AND jsonb_array_length(selector->'expected') NOT BETWEEN 1 AND 32 OR EXISTS(SELECT 1 FROM jsonb_array_elements_text(selector->'expected') value WHERE value!~'^[a-z][a-z0-9_.:-]{0,63}$') THEN RAISE EXCEPTION 'naming corroboration invalid'; END IF;
  END LOOP;
  FOR proposed IN SELECT value FROM jsonb_array_elements(p->'proposedAssignments') LOOP
    IF jsonb_typeof(proposed)<>'object' OR (SELECT count(*) FROM jsonb_object_keys(proposed))<>2 OR NOT(proposed ?& ARRAY['dimensionRef','definitionRef']) OR proposed->>'dimensionRef'!~'^dimension_[a-f0-9]{24}$' OR proposed->>'definitionRef'!~'^category_[a-f0-9]{24}$' THEN RAISE EXCEPTION 'naming proposal invalid'; END IF;
  END LOOP;
...
BEGIN
  ...
  IF NOT FOUND OR (r.ad_account_id,r.template_ref,r.revision) IS DISTINCT FROM (NEW.ad_account_id,NEW.template_ref,NEW.version) THEN RAISE EXCEPTION 'naming template head identity invalid'; END IF;
