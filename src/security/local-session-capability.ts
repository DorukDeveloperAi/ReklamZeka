...
function validClaims(value: unknown): LocalSessionClaims {
  ...
  if (value.version !== 1 || (value.kind !== "bootstrap" && value.kind !== "session")
    ...
    || !Array.isArray(value.scopes) || value.scopes.length < 1 || value.scopes.length > 14
  ...
  const expected = value.kind === "bootstrap" ? ["local_session:bootstrap"]
    : ["approval_queue:decide", "approval_queue:read", "autonomy_rules:draft", "autonomy_rules:read", "policy_bundle:draft", "policy_bundle:read", "budget_lab:draft", "budget_lab:read", "decision_room:mark_read", "decision_room:read", "practice_lab:read", "promotion_catalog:read", "promotion_preflight:read", "promotion_proposal:draft"];
...
export function mintLocalSessionCapability(input: Readonly<{
  ...
  const claims = validClaims({
    ...
    scopes: input.kind === "bootstrap"
      ...
      : ["approval_queue:decide", "approval_queue:read", "autonomy_rules:draft", "autonomy_rules:read", "policy_bundle:draft", "policy_bundle:read", "budget_lab:draft", "budget_lab:read", "decision_room:mark_read", "decision_room:read", "practice_lab:read", "promotion_catalog:read", "promotion_preflight:read", "promotion_proposal:draft"] as const,
