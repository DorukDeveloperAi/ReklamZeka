...
export type LocalSessionScope = "approval_queue:decide" | "approval_queue:read" | "budget_lab:draft" | "budget_lab:read" | "decision_room:read" | "decision_room:mark_read" | "practice_lab:read" | "promotion_catalog:read" | "promotion_preflight:read" | "promotion_proposal:draft" | "local_session:bootstrap";
...
const SCOPES = new Set<LocalSessionScope>([
  "approval_queue:decide", "approval_queue:read", "budget_lab:draft", "budget_lab:read", "decision_room:read", "decision_room:mark_read", "practice_lab:read", "promotion_catalog:read", "promotion_preflight:read", "promotion_proposal:draft", "local_session:bootstrap",
]);
...
function validClaims(value: unknown): LocalSessionClaims {
  ...
  if (value.version !== 1 || (value.kind !== "bootstrap" && value.kind !== "session")
    ...
    || !Array.isArray(value.scopes) || value.scopes.length < 1 || value.scopes.length > 10
  ...
  const expected = value.kind === "bootstrap" ? ["local_session:bootstrap"]
    : ["approval_queue:decide", "approval_queue:read", "budget_lab:draft", "budget_lab:read", "decision_room:mark_read", "decision_room:read", "practice_lab:read", "promotion_catalog:read", "promotion_preflight:read", "promotion_proposal:draft"];
...
export function mintLocalSessionCapability(input: Readonly<{
  ...
  const claims = validClaims({
    ...
    scopes: input.kind === "bootstrap"
      ...
      : ["approval_queue:decide", "approval_queue:read", "budget_lab:draft", "budget_lab:read", "decision_room:mark_read", "decision_room:read", "practice_lab:read", "promotion_catalog:read", "promotion_preflight:read", "promotion_proposal:draft"] as const,
