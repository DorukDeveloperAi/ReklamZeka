...
export type LocalSessionScope = "budget_lab:draft" | "budget_lab:read" | "decision_room:read" | "decision_room:mark_read" | "practice_lab:read" | "local_session:bootstrap";
...
const SCOPES = new Set<LocalSessionScope>([
  "budget_lab:draft", "budget_lab:read", "decision_room:read", "decision_room:mark_read", "practice_lab:read", "local_session:bootstrap",
]);
...
function validClaims(value: unknown): LocalSessionClaims {
  ...
  if (value.version !== 1 || (value.kind !== "bootstrap" && value.kind !== "session")
    ...
    || !Array.isArray(value.scopes) || value.scopes.length < 1 || value.scopes.length > 5
  ...
  const expected = value.kind === "bootstrap" ? ["local_session:bootstrap"]
    : ["budget_lab:draft", "budget_lab:read", "decision_room:mark_read", "decision_room:read", "practice_lab:read"];
...
export function mintLocalSessionCapability(input: Readonly<{
  ...
  const claims = validClaims({
    ...
    scopes: input.kind === "bootstrap"
      ...
      : ["budget_lab:draft", "budget_lab:read", "decision_room:mark_read", "decision_room:read", "practice_lab:read"] as const,
