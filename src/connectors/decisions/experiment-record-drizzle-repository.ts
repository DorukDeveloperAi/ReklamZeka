...
export class DrizzleExperimentRecordRepository {
  ...
  async plan(input: Readonly<{ workspaceId: string; actorId: string; actorRef: string; role: "owner" | "admin" | "analyst";
    ...
    return this.database.transaction(async (transaction) => {
      ...
      const existing = rows<{ record_hash: unknown }>(await tx.execute(sql`select record_hash from experiment_record_revisions where workspace_id = ${input.workspaceId}::uuid and experiment_ref = ${ref} order by sequence desc limit 1 for update`));
      if (existing[0]) return Object.freeze({ experimentRef: ref, recordHash: String(existing[0]!.record_hash), outcome: "unchanged" as const, capabilities: Object.freeze({ canPublish: false as const, canApprove: false as const, canExecute: false as const, canWriteMeta: false as const }) });
  ...
  async recordOutcome(input: Readonly<{ workspaceId: string; actorId: string; actorRef: string; role: "owner" | "admin" | "analyst";
    ...
    return this.database.transaction(async (transaction) => {
      ...
      if (head.length !== 1) fail("not_found");
