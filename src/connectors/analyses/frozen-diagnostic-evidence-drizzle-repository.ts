...
export class FrozenDiagnosticEvidenceRepositoryError extends Error {
  constructor(
    readonly code: "insufficient_evidence" | "corrupt_store",
    readonly diagnosticStage?: "lookup" | "insert",
  ) {
...
export class DrizzleFrozenDiagnosticEvidenceRepository {
  async saveInTransaction(database: Database, input: Readonly<{ contextId: string; context: EffectiveCampaignContext }>): Promise<"inserted" | "unchanged"> {
    ...
    let found: readonly { feature_ref: string; feature_hash: string; window_ref: string; window_hash: string; context_hash: string; captured_at: string }[];
    try { found = rows<{ feature_ref: string; feature_hash: string; window_ref: string; window_hash: string; context_hash: string; captured_at: string }>(await database.execute(sql`
      select feature.feature_ref, feature.feature_hash, l3_window.window_ref, l3_window.window_hash, context.context_hash, context.captured_at::text
      from effective_campaign_contexts context
      join deterministic_feature_snapshots feature on feature.workspace_id = context.workspace_id
        and feature.feature_ref in (select value from jsonb_array_elements_text(${JSON.stringify(context.data.featureRefs)}::jsonb))
      join deterministic_window_snapshots l3_window on l3_window.workspace_id = context.workspace_id
        and l3_window.window_ref in (select value from jsonb_array_elements_text(${JSON.stringify(context.data.windowRefs)}::jsonb))
      where context.workspace_id = ${context.workspaceId}::uuid and context.id = ${contextId}::uuid
        ...
        and l3_window.meta_connection_id = context.meta_connection_id and l3_window.ad_account_id = context.ad_account_id
      order by feature.feature_ref, l3_window.window_ref
    `)); } catch (error) {
      if (error instanceof FrozenDiagnosticEvidenceRepositoryError) throw error;
      throw new FrozenDiagnosticEvidenceRepositoryError("corrupt_store", "lookup");
    }
    ...
    try { result = rows<{ id: string }>(await database.execute(sql`
    ...
    `)); } catch (error) {
      if (error instanceof FrozenDiagnosticEvidenceRepositoryError) throw error;
      throw new FrozenDiagnosticEvidenceRepositoryError("corrupt_store", "insert");
    }
