...
export type OperationReadRepository = Readonly<{ load(input: Readonly<{ workspaceId: string; period: OperationPeriod; sliceRef: string | null; limit: number; cursor: string | null }>): Promise<Readonly<{ facts: readonly OperationRowFact[]; unavailable: boolean; nextCursor: string | null }>> }>;
...
export class OperationReadService {
  ...
  async read(workspaceId: string, input: OperationReadRequest = {}): Promise<OperationReadProjection & Readonly<{ nextCursor: string | null }>> {
    ...
    const period = operationPeriod({ kind: input.period, startDate: input.start, endDate: input.end, now: this.now() });
