...
export type OperationalTimelineEvent = Readonly<{
  kind: "slice_rule_draft" | "budget_proposal" | "budget_selection" | "action_preparation" | "delivery_alert" | "approval_proposed" | "approval_decision" | "temporal_evaluation";
  occurredAt: string;
  title: string;
  detail: string;
}>;
...
function safe(event: OperationalTimelineEvent): OperationalTimelineEvent {
  if (!event || typeof event !== "object" || !["slice_rule_draft", "budget_proposal", "budget_selection", "action_preparation", "delivery_alert", "approval_proposed", "approval_decision", "temporal_evaluation"].includes(event.kind)
