...
export type OperationalTimelineEvent = Readonly<{
  kind: "slice_rule_draft" | "delivery_alert" | "approval_proposed" | "approval_decision";
  occurredAt: string;
  title: string;
  detail: string;
}>;
...
function safe(event: OperationalTimelineEvent): OperationalTimelineEvent {
  if (!event || typeof event !== "object" || !["slice_rule_draft", "delivery_alert", "approval_proposed", "approval_decision"].includes(event.kind)
