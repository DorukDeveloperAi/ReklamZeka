...
export type PlaybookInput = Readonly<{ title: string; body: string; sourceOptionId: string }>;
...
function playbookInput(value: PlaybookInput): PlaybookInput {
  ...
  const sourceOptionId = typeof value?.sourceOptionId === "string" ? value.sourceOptionId.trim() : "";
  if (!title || title.length > 240 || CONTROL.test(title) || PRIVATE_IDENTIFIER.test(title) || !body || body.length > 16_000 || CONTROL.test(body) || PRIVATE_IDENTIFIER.test(body)
    || !SOURCE_OPTION.test(sourceOptionId)) throw new Error("invalid_input");
  return Object.freeze({ title, body, sourceOptionId });
}
...
export class SkillCatalogService {
  ...
  async list(principal: TrustedDecisionRoomPrincipal) {
    ...
    return Object.freeze({ contractVersion: "skill-catalog-ui/1.0.0", activeProfile: items.find((item) => item.kind === "profile") ?? null,
      playbooks: items.filter((item) => item.kind === "playbook"), sources: await this.repo.sources(principal.workspaceId),
