...
export type ApprovalQueueAgentCall =
  | Readonly<{ name: "approval_queue_list"; arguments: Readonly<{ entityRef?: string | null; campaignRef?: string | null; limit?: number; cursor?: string | null }> }>
...
export class ApprovalQueueAgentContract {
  ...
  async execute(principal: TrustedDecisionRoomPrincipal, call: ApprovalQueueAgentCall) {
    ...
    exact(call.arguments, call.name === "approval_queue_list" ? ["entityRef", "campaignRef", "limit", "cursor"] : ["unitRef"]);
...
export const APPROVAL_QUEUE_AGENT_TOOLS = Object.freeze([
  Object.freeze({ name: "approval_queue_list", description: "List public-safe approval queue units for the server-bound workspace; no decision or execution authority.",
    inputSchema: Object.freeze({ type: "object", additionalProperties: false, properties: Object.freeze({ entityRef: Object.freeze({ type: ["string", "null"], pattern: "^entity_[a-f0-9]{16}$" }), campaignRef: Object.freeze({ type: ["string", "null"], pattern: "^entity_[a-f0-9]{16}$" }), limit: Object.freeze({ type: "integer", minimum: 1, maximum: 100 }), cursor: Object.freeze({ type: ["string", "null"] }) }) }) }),
