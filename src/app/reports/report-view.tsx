...
export function ReportView({
  ...
  return <><a className="skip-link" href="#report-content">Rapor içeriğine geç</a><main id="report-content" tabIndex={-1} className="report-shell">
    ...
    <section className="report-source" aria-label="Rapor kaynağı" role="status" aria-live="polite"><span>Snapshot: {report.snapshotId}</span><span>{report.source.currency} · {report.source.timezone}</span><span>Veri {Math.round(report.source.freshness.hours ?? 0)} saat önce güncellendi</span><span>{expiryText}</span></section>
...
export function ReportUnavailable({ reason }: { reason: "expired" | "revoked" | "invalid" | "configuration" }) {
  ...
  return <><a className="skip-link" href="#report-content">Rapor içeriğine geç</a><main id="report-content" tabIndex={-1} className="report-shell report-unavailable">
