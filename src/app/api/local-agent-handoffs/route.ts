...
import { localAgentHandoffNotConfiguredResponse } from "@/server/local-agent-handoff-http";
...
function handlers() {
  try {
    ...
    return createLocalAgentCoordinationHandlers({ database, config }).handoffs;
  } catch { return null; }
}

export function POST(request?: Request) {
  const found = handlers();
  return found && request ? found.POST(request) : localAgentHandoffNotConfiguredResponse();
}
export function PATCH(request?: Request) {
  const found = handlers();
  return found && request ? found.PATCH(request) : localAgentHandoffNotConfiguredResponse();
}
