...
import { createLocalPracticeLabRouteHandler, practiceLabNotConfiguredResponse } from "@/server/local-practice-lab-runtime";
...
function configuredHandler() {
  try {
    ...
    return createLocalPracticeLabRouteHandler({ database: runtimeDatabase, config });
  } catch {
    return null;
  }
}
...
export function GET(request?: Request) {
  ...
  return handler && request ? handler(request) : practiceLabNotConfiguredResponse();
}
