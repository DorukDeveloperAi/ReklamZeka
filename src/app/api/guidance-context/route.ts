...
import { guidanceAgentNotConfiguredResponse } from "@/server/guidance-agent-http";
import { createLocalGuidanceAgentHandlers } from "@/server/local-guidance-agent-runtime";
...
function handlers() { try {
  ...
  return createLocalGuidanceAgentHandlers({ database, config });
} catch { return null; } }
export function GET(request?: Request) { const found = handlers(); return found && request ? found.GET(request) : guidanceAgentNotConfiguredResponse(); }
export function POST(request?: Request) { const found = handlers(); return found && request ? found.POST(request) : guidanceAgentNotConfiguredResponse(); }
