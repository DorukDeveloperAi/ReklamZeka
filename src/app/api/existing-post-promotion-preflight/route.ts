...
function configuredCatalogHandler() {
  try {
    const environment = {
    ...
    return createLocalExistingPostPromotionCatalogRouteHandler({ database: runtimeDatabase, config });
  } catch {
    return null;
  }
}
...
export function GET(request?: Request) {
  const handler = configuredCatalogHandler();
  return handler && request ? handler(request) : existingPostPromotionCatalogNotConfiguredResponse();
}
