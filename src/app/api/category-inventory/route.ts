...
import { categoryInventoryNotConfiguredResponse } from "@/server/category-inventory-http";
import { createLocalCategoryInventoryHandler } from "@/server/local-category-inventory-runtime";
...
function handler() { try {
  ...
  return createLocalCategoryInventoryHandler({ database, config });
} catch { return null; } }
export function GET(request?: Request) { const found = handler();
  return found && request ? found(request) : categoryInventoryNotConfiguredResponse(); }
