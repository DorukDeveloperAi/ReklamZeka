...
import { createLocalProgressiveFormalizationHandlers } from "@/server/local-progressive-formalization-runtime";
import { localDecisionRoomConfig } from "@/server/local-decision-room-runtime";
import { progressiveFormalizationNotConfiguredResponse } from "@/server/progressive-formalization-http";
...
function handlers() { try {
  ...
  if (!database) { const pool = new Pool({ connectionString: environment.DATABASE_URL, max: 2,
    connectionTimeoutMillis: 5_000, statement_timeout: 20_000, idleTimeoutMillis: 30_000, allowExitOnIdle: true });
    pool.on("error", () => undefined); database = drizzle(pool, { schema }); }
  return createLocalProgressiveFormalizationHandlers({ database, config });
} catch { return null; } }

export function GET(request?: Request) {
  const found = handlers(); return found && request ? found.GET(request) : progressiveFormalizationNotConfiguredResponse();
}
export function POST(request?: Request) {
  const found = handlers(); return found && request ? found.POST(request) : progressiveFormalizationNotConfiguredResponse();
}
