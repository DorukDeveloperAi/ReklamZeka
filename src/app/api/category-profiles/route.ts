...
import { categoryProfileNotConfiguredResponse } from "@/server/category-profile-lifecycle-http";
...
function handlers() { try {
  ...
  return createLocalCategoryProfileHandlers({ database, config });
} catch { return null; } }

export function GET(request?: Request) {
  const found = handlers(); return found && request ? found.GET(request) : categoryProfileNotConfiguredResponse();
}
export function POST(request?: Request) {
  const found = handlers(); return found && request ? found.POST(request) : categoryProfileNotConfiguredResponse();
}
