...
import { starterCategoryAdoptionNotConfiguredResponse } from "@/server/starter-category-adoption-http";
...
function handlers() { try {
  ...
  return createLocalStarterCategoryAdoptionHandlers({ database, config });
} catch { return null; } }
export function GET(request?: Request) { const found = handlers();
  return found && request ? found.GET(request) : starterCategoryAdoptionNotConfiguredResponse(); }
export function POST(request?: Request) { const found = handlers();
  return found && request ? found.POST(request) : starterCategoryAdoptionNotConfiguredResponse(); }
