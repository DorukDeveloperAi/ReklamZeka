...
import { createLocalDeliveryHealthAlertLedgerHandlers } from "@/server/local-delivery-health-alert-ledger-runtime";
import { deliveryHealthAlertNotConfiguredResponse } from "@/server/delivery-health-alert-ledger-http";
...
function handlers() {
  try {
    ...
    if (!database) {
      const pool = new Pool({ connectionString: environment.DATABASE_URL, max: 2, connectionTimeoutMillis: 5_000,
        statement_timeout: 15_000, idleTimeoutMillis: 30_000, allowExitOnIdle: true });
      pool.on("error", () => undefined);
      database = drizzle(pool, { schema });
    }
    return createLocalDeliveryHealthAlertLedgerHandlers({ database, config });
  } catch { return null; }
}

export function GET(request?: Request) {
  const found = handlers();
  return found && request ? found.GET(request) : deliveryHealthAlertNotConfiguredResponse();
}
export function POST(request?: Request) {
  const found = handlers();
  return found && request ? found.POST(request) : deliveryHealthAlertNotConfiguredResponse();
}
