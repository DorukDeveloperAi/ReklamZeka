...
import { guidanceStudioNotConfiguredResponse } from "@/server/guidance-studio-http";
...
function handlers() { try {
  ...
  return createLocalGuidanceStudioHandlers({ database, config });
} catch { return null; } }
export function GET(request?: Request) { const found = handlers(); return found && request ? found.GET(request) : guidanceStudioNotConfiguredResponse(); }
export function POST(request?: Request) { const found = handlers(); return found && request ? found.POST(request) : guidanceStudioNotConfiguredResponse(); }
export function PATCH(request?: Request) { const found = handlers(); return found && request ? found.PATCH(request) : guidanceStudioNotConfiguredResponse(); }
