...
import { categoryArchiveImpactNotConfiguredResponse } from "@/server/category-archive-impact-http";
import { createLocalCategoryArchiveImpactHandler } from "@/server/local-category-archive-impact-runtime";
import { localDecisionRoomConfig } from "@/server/local-decision-room-runtime";

export const dynamic = "force-dynamic"; export const runtime = "nodejs";
let database: ReturnType<typeof drizzle<typeof schema>> | null = null;
function handler() { try {
  ...
  if (!database) { const pool = new Pool({ connectionString: environment.DATABASE_URL, max: 2,
    connectionTimeoutMillis: 5_000, statement_timeout: 15_000, idleTimeoutMillis: 30_000, allowExitOnIdle: true });
    pool.on("error", () => undefined); database = drizzle(pool, { schema }); }
  return createLocalCategoryArchiveImpactHandler({ database, config });
} catch { return null; } }
export function GET(request?: Request) { const found = handler();
  return found && request ? found(request) : categoryArchiveImpactNotConfiguredResponse(); }
