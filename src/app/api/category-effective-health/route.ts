...
import { categoryEffectiveHealthNotConfiguredResponse } from "@/server/category-effective-health-http";
import { createLocalCategoryEffectiveHealthHandler } from "@/server/local-category-effective-health-runtime";
...
function handler() { try {
  ...
  return createLocalCategoryEffectiveHealthHandler({ database, config });
} catch { return null; } }
export function GET(request?: Request) { const found = handler();
  return found && request ? found(request) : categoryEffectiveHealthNotConfiguredResponse(); }
