...
import { createLocalExperimentRecordHandler, experimentRecordNotConfiguredResponse } from "@/server/local-experiment-record-runtime";
...
function handler() { try {
  ...
  if (!database) { const pool = new Pool({ connectionString: environment.DATABASE_URL, max: 2, connectionTimeoutMillis: 5_000,
    statement_timeout: 10_000, idleTimeoutMillis: 30_000, allowExitOnIdle: true }); pool.on("error", () => undefined); database = drizzle(pool, { schema }); }
  return createLocalExperimentRecordHandler({ database, config });
} catch { return null; } }
export function POST(): ReturnType<typeof experimentRecordNotConfiguredResponse>;
export function POST(request: Request): Promise<Response> | Response;
export function POST(request?: Request) { const configured = handler(); return configured && request ? configured(request) : experimentRecordNotConfiguredResponse(); }
