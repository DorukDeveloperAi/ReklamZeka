...
import { createLocalSliceRuleWorkspaceHandlers } from "@/server/local-slice-rule-workspace-runtime";
import { sliceRuleWorkspaceNotConfiguredResponse } from "@/server/slice-rule-workspace-http";
...
function handlers() {
  try {
    ...
    return createLocalSliceRuleWorkspaceHandlers({ database, config });
  } catch { return null; }
}

export function GET(request?: Request) {
  const found = handlers();
  return found && request ? found.GET(request) : sliceRuleWorkspaceNotConfiguredResponse();
}

export function POST(request?: Request) {
  const found = handlers();
  return found && request ? found.POST(request) : sliceRuleWorkspaceNotConfiguredResponse();
}
