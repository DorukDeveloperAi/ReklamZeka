...
import {
  localAgentSessionNotConfiguredResponse,
  ...
  try {
    ...
    if (!database) {
      const pool = new Pool({ connectionString: environment.DATABASE_URL, max: 2,
        connectionTimeoutMillis: 5_000, statement_timeout: 10_000, idleTimeoutMillis: 30_000,
        allowExitOnIdle: true });
      pool.on("error", () => undefined);
      database = drizzle(pool, { schema });
    }
    return createLocalAgentCoordinationHandlers({ database, config }).sessions;
  } catch { return null; }
}

export function GET(request?: Request) {
  const found = handlers();
  return found && request ? found.GET(request) : localAgentSessionNotConfiguredResponse();
}
export function POST(request?: Request) {
  const found = handlers();
  return found && request ? found.POST(request) : localAgentSessionNotConfiguredResponse();
}
export function PATCH(request?: Request) {
  const found = handlers();
  return found && request ? found.PATCH(request) : localAgentSessionNotConfiguredResponse();
}
