...
import { instructionPolicyImpactNotConfiguredResponse } from "@/server/instruction-policy-impact-http";
...
function handler() { try {
  ...
  return createLocalInstructionPolicyImpactHandler({ database, config });
} catch { return null; } }
export function GET(request?: Request) { const found = handler();
  return found && request ? found(request) : instructionPolicyImpactNotConfiguredResponse(); }
