...
import {
  createLocalPortfolioCapabilityRouteHandler,
  portfolioCapabilityNotConfiguredResponse,
} from "@/server/local-portfolio-capability-runtime";
...
function handler() {
  try {
    ...
    const config = localDecisionRoomConfig(env);
    if (!config) return null;
    if (!database) database = drizzle(new Pool({
    ...
    return createLocalPortfolioCapabilityRouteHandler({ database, config });
  } catch {
    return null;
  }
}

export function GET(): ReturnType<typeof portfolioCapabilityNotConfiguredResponse>;
export function GET(request: Request): Promise<Response> | Response;
export function GET(request?: Request) {
  const configured = handler();
  return configured && request ? configured(request) : portfolioCapabilityNotConfiguredResponse();
}
