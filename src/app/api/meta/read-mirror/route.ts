...
import {
  createLocalMetaReadMirrorRouteHandler,
  metaReadMirrorNotConfiguredResponse,
} from "@/server/local-meta-read-mirror-runtime";
...
function handler() {
  try {
    ...
    return createLocalMetaReadMirrorRouteHandler({ database, config });
  } catch {
    return null;
  }
}

export function GET(): ReturnType<typeof metaReadMirrorNotConfiguredResponse>;
export function GET(request: Request): Promise<Response> | Response;
export function GET(request?: Request) {
  const configured = handler();
  return configured && request ? configured(request) : metaReadMirrorNotConfiguredResponse();
}
