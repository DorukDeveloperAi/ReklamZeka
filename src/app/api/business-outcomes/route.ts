...
import { createLocalBusinessOutcomeSignalHandler, businessOutcomeSignalNotConfiguredResponse } from "@/server/local-business-outcome-signal-runtime";
...
function handler() { try {
  ...
  return createLocalBusinessOutcomeSignalHandler({ database, config });
} catch { return null; } }
...
export function POST(request?: Request) { const configured = handler(); return configured && request ? configured(request) : businessOutcomeSignalNotConfiguredResponse(); }
