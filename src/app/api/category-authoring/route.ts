...
import { categoryAuthoringNotConfiguredResponse } from "@/server/category-authoring-http";
...
function handlers() { try {
  ...
  return createLocalCategoryAuthoringHandlers({ database, config });
} catch { return null; } }

export function GET(request?: Request) {
  const found = handlers(); return found && request ? found.GET(request) : categoryAuthoringNotConfiguredResponse();
}
export function POST(request?: Request) {
  const found = handlers(); return found && request ? found.POST(request) : categoryAuthoringNotConfiguredResponse();
}
