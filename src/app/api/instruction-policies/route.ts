...
import { instructionPolicyNotConfiguredResponse } from "@/server/instruction-policy-lifecycle-http";
import { createLocalInstructionPolicyHandlers } from "@/server/local-instruction-policy-runtime";
...
function handlers() { try {
  ...
  return createLocalInstructionPolicyHandlers({ database, config });
} catch { return null; } }

export function GET(request?: Request) {
  const found = handlers(); return found && request ? found.GET(request) : instructionPolicyNotConfiguredResponse();
}
export function POST(request?: Request) {
  const found = handlers(); return found && request ? found.POST(request) : instructionPolicyNotConfiguredResponse();
}
