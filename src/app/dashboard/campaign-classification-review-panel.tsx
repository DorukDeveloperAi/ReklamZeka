...
type Entry = Readonly<{ campaignRef: string; name: string; accountName: string; fetchedAt: string; facets: readonly Facet[]; reviewRequired: boolean; sliceCandidate: "ready" | "review_required"; reasonCodes: readonly string[] }>;
type Snapshot = Readonly<{ version: "campaign-classification-review/1.0.0"; entries: readonly Entry[]; summary: Readonly<{ campaigns: number; reviewRequired: number; sliceCandidates: number }>; authority: Readonly<{ canAssign: false; canPublish: false; canAuthorizeAction: false; canWriteMeta: false }> }>;
...
function parse(value: unknown): Snapshot | null {
  ...
  if (candidate.version !== "campaign-classification-review/1.0.0" || !Array.isArray(candidate.entries)
    || !candidate.entries.every((entry) => entry && typeof entry === "object" && ENTITY_REF.test((entry as Record<string, unknown>).campaignRef as string)
      && ["ready", "review_required"].includes((entry as Record<string, unknown>).sliceCandidate as string))
    || !candidate.summary || typeof (candidate.summary as Record<string, unknown>).campaigns !== "number"
    || typeof (candidate.summary as Record<string, unknown>).reviewRequired !== "number"
    || typeof (candidate.summary as Record<string, unknown>).sliceCandidates !== "number" || !candidate.authority
...
export function CampaignClassificationReviewPanel(props: Readonly<{ onPrepareAssignment?(handoff: CategoryAssignmentHandoff): void; onOpenSlicePreparation?(campaignRef: string): void; onOpenCategorySetup?(): void }> = {}) {
  ...
  return <section className={styles.panel}>
    ...
    {state === "unavailable" || !snapshot ? state !== "loading" ? <p>Güvenli inceleme kaynağı şu anda kullanılamıyor.</p> : null : <>
      <p><strong>{snapshot.summary.reviewRequired}</strong> / {snapshot.summary.campaigns} kampanya insan incelemesi bekliyor. <strong>{snapshot.summary.sliceCandidates}</strong> kapsam, yalnız tutarlı kanıtla Slice hazırlığına aktarılabilir.</p>
      ...
      {visibleEntries.length ? <div className={styles.campaignGrid}>{visibleEntries.map((entry) => <article key={entry.campaignRef} className={styles.campaignCard}><header><strong>{entry.name}</strong><small>{entry.accountName} · {entry.reviewRequired ? "İnceleme gerekli" : "Kayıtlı"}</small></header><dl>{entry.facets.map((item) => <div key={item.facet}><dt>{labels[item.facet] ?? item.facet}</dt><dd>{item.state === "assigned" ? item.values.join(", ") : item.state === "conflict" ? `Çelişki: ${item.reasonCodes.join(", ")}` : item.state === "not_configured" ? "Kategori boyutu tanımlı değil" : "Eksik"}<small> · kanıt: {item.evidenceCount}</small>{item.state === "missing" && handoffFacet(item.facet) ? <button type="button" onClick={() => beginManualAssignment(entry, item.facet)}>Manuel atamayı hazırla</button> : null}</dd></div>)}</dl><div className={styles.classificationHandoff}><strong>{entry.sliceCandidate === "ready" ? "Slice hazırlığına uygun" : "Slice için inceleme gerekli"}</strong><small>{entry.sliceCandidate === "ready" ? "Yerli/yabancı sınırı ile zorunlu kapsam kanıtı tutarlı. Aktarım yeni kural veya atama oluşturmaz." : "Eksik ya da çelişkili kanıt varken kapsam tahmini ve Slice aktarımı kapalıdır."}</small>{entry.sliceCandidate === "ready" && props.onOpenSlicePreparation ? <button type="button" onClick={() => props.onOpenSlicePreparation?.(entry.campaignRef)}>Slice hazırlığını aç</button> : null}</div>{entry.reasonCodes.length ? <small>İnceleme sinyali: {entry.reasonCodes.join(", ")}</small> : null}</article>)}</div> : <p>Bu arama ve inceleme filtresinde kampanya yok. Başka kaynakla doldurulmadı.</p>}
