...
type Item = Readonly<{ guideId: string; guideRef: string; label: string; revisionId: string; activeRevisionId: string | null; headVersion: number; revision: number; revisionHash: string; interpretationHash: string; interpretationAccepted: boolean; sliceRef: string; market: "yerli" | "yabanci"; mode: string; freeText: string; schedule: Readonly<{ frequency: string; timezone: string; localTime: string }>; actionAllowlist: readonly string[]; budgetRefs: readonly Readonly<{ limitRef: string; scopeKind: string }>[]; rollbackConditions: readonly string[]; createdAt: string }>;
...
function parse(value: unknown): Snapshot {
  ...
  for (const item of value.items) if (!record(item) || !keys(item, ["guideId", "guideRef", "label", "revisionId", "activeRevisionId", "headVersion", "revision", "revisionHash", "interpretationHash", "interpretationAccepted", "sliceRef", "market", "mode", "freeText", "schedule", "actionAllowlist", "budgetRefs", "rollbackConditions", "createdAt"])
    ...
    || !Array.isArray(item.actionAllowlist) || item.actionAllowlist.length > 7 || item.actionAllowlist.some((action) => !["status_pause", "status_activate", "budget_decrease", "budget_increase", "campaign_rename", "adset_rename", "ad_rename"].includes(String(action)))
    || !Array.isArray(item.budgetRefs) || item.budgetRefs.length > 64 || item.budgetRefs.some((budget) => !record(budget) || !keys(budget, ["limitRef", "scopeKind"]) || typeof budget.limitRef !== "string" || !REF.test(budget.limitRef) || !["market", "organization_campaign", "geo_targeting_platform", "campaign_ad_set"].includes(String(budget.scopeKind)))
    || !Array.isArray(item.rollbackConditions) || item.rollbackConditions.length > 64 || item.rollbackConditions.some((condition) => typeof condition !== "string" || condition.length > 500)
...
export function GuideLifecyclePanel(props: Readonly<{ onSessionRequiredChange?: (required: boolean) => void }>) {
  ...
  const create = async () => { setBusy(true); setError(null); try { const schedule = revising ? { ...revising.schedule, localTime: form.localTime } : { frequency: "daily", timezone: "Europe/Istanbul", localTime: form.localTime }; const body = revising
  ...
  return <section className={styles.panel} aria-label="Kanonik Kılavuz yaşam döngüsü">
    ...
    <div className={styles.guidanceFields}>
      <label>Ad<input maxLength={160} value={form.label} disabled={busy || !snapshot?.authority.canDraft || revising !== null} onChange={(event) => setForm({ ...form, label: event.target.value })} /></label>
      ...
      <button className={styles.primaryButton} type="button" disabled={busy || !snapshot?.authority.canDraft || !form.label.trim() || !form.sliceRef.trim() || !form.freeText.trim()} onClick={() => void create()}>{revising ? "Yeni revision taslağını kaydet" : "İnsan onaylı durum Kılavuzu taslağı oluştur"}</button>
      {revising ? <button type="button" disabled={busy} onClick={() => { setRevising(null); setForm({ label: "", sliceRef: "", market: "yerli", freeText: "", localTime: "09:00" }); }}>Revision düzenlemeyi iptal et</button> : null}
