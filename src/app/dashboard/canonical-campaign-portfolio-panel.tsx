...
export function CanonicalCampaignPortfolioPanel({ projection, onOpenAgentContext }: Readonly<{
  ...
  return <section className={styles.splitWorkspace} aria-label="Kanonik Meta kampanya portföyü">
    <section className={styles.panel}><header className={styles.panelHeader}><div><span className={styles.kicker}>CANONICAL META PORTFÖYÜ · SALT-OKUNUR</span><h2>Kampanyalar</h2></div><span className={styles.statusPill} data-tone="good">{entries.length} kampanya</span></header><p>Yerel oturumla bağlanmış Meta aynası. Filtre, öneri, policy veya Meta write içermez.</p><div className={styles.selectorList}>{entries.map((entry) => <button key={entry.campaignRef} data-active={selected.campaignRef === entry.campaignRef} type="button" onClick={() => setSelectedRef(entry.campaignRef)}><span><strong>{entry.name}</strong><small>{entry.accountName} · {entry.objective ?? "Amaç bilinmiyor"}</small></span><span className={styles.statusPill} data-tone="neutral">{entry.status ?? "Durum bilinmiyor"}</span></button>)}</div></section>
    <section className={styles.panel}><header className={styles.detailHeader}><div><span className={styles.kicker}>CANONICAL CAMPAIGN CONTEXT</span><h2>{selected.name}</h2><p>{selected.accountName} · {selected.objective ?? "Meta amacı bilinmiyor"} · {selected.status ?? "Durum bilinmiyor"}</p></div><button type="button" onClick={() => onOpenAgentContext(selected.campaignRef, selected.name)}>Agent ile aç ✦</button></header>
      ...
      <section className={styles.demoHierarchyDrilldown}><header><div><span className={styles.kicker}>HİYERARŞİ · KANONİK AYNADAN</span><h3>Mevcut Meta kurulumu</h3></div><span className={styles.statusPill} data-tone="good">read-only</span></header><details open><summary><span>Hesap</span><strong>{selected.accountName}</strong><small>{selected.currency}</small></summary><details open><summary><span>Kampanya</span><strong>{selected.name}</strong><small>{selected.campaign.status ?? "Durum bilinmiyor"}</small></summary>{selected.campaign.adSets.map((adSet) => <details key={adSet.adSetRef}><summary><span>Ad set</span><strong>{adSet.name}</strong><small>{adSet.status ?? "Durum bilinmiyor"} · {adSet.ads.length} reklam</small></summary>{adSet.ads.map((ad) => <div className={styles.demoCreativeLeaf} key={ad.adRef}><span>Ad / creative</span><strong>{ad.name}</strong><small>{ad.status ?? "Durum bilinmiyor"} · {ad.creative?.sourceType ?? "creative yok"}</small></div>)}</details>)}</details></details></section>
