...
export function NormalizationWorkbenchPanel({ initialCampaignIntentTemplate = "" }: Readonly<{
  ...
  return <section className={`${styles.surface} ${styles.editor}`} aria-label="Talimat normalizasyon çalışma alanı">
    ...
    {loading ? <p>Normalizasyon kayıtları yükleniyor…</p> : message && !snapshot ? <p role="alert">{message}</p> : <>
      ...
      <section className={styles.briefNextDecision} aria-label="Yapılandırılmış taslak kontrolü"><span>YAPILANDIRILMIŞ TASLAK KONTROLÜ · DRAFT-ONLY</span>
        <strong>Sonraki insan incelemesi için eksik kararları görün</strong><small>Bu önizleme yalnız eksik alanları ve insan incelemeli clause özetini üretir; strict policy oluşturmaz, yayınlamaz veya onaya göndermez.</small>
        <div className={styles.split}><label>Niyet<select aria-label="Taslak niyeti" value={policyIntent} disabled={saving} onChange={(event) => { setPolicyIntent(event.target.value as PolicyIntent); setAssessment(null); }}>
          ...
          <label>Kapsam<select aria-label="Taslak kapsamı" value={policyScope} disabled={saving} onChange={(event) => { setPolicyScope(event.target.value as PolicyScope); setAssessment(null); }}><option value="global">Çalışma alanı geneli</option><option value="specific">Belirli kapsam</option></select></label>
