...
import { ExistingPostPromotionProposalDraftService, ExistingPostPromotionDraftError } from "@/application/existing-post-promotion-proposal-draft-service";
...
export function createLocalExistingPostPromotionProposalDraftRouteHandler(input: Readonly<{ database: Pick<Database, "execute">; config: LocalDecisionRoomConfig }>) {
  return async function POST(request: Request) { try {
    ...
    { submitResolved: async () => { throw new ExistingPostPromotionDraftError("material_unavailable"); } }, [bound.membership]);
