...
import { businessOutcomeReadNotConfiguredResponse, createBusinessOutcomeReadHttpHandler } from "@/server/business-outcome-read-http";
import { resolveTrustedLocalBusinessOutcomePrincipal, resolveTrustedLocalBusinessOutcomeReadPrincipal, type LocalDecisionRoomConfig } from "@/server/local-decision-room-runtime";
...
export function createLocalBusinessOutcomeReadHandler(input: Readonly<{ database: Pick<Database, "execute">; config: LocalDecisionRoomConfig }>) {
  return async function GET(request: Request) { try {
    const bound = await resolveTrustedLocalBusinessOutcomeReadPrincipal({ request, database: input.database, config: input.config });
    return createBusinessOutcomeReadHttpHandler({ service: new BusinessOutcomeReadService(new DrizzleBusinessOutcomeSignalRepository(input.database as never), [bound.membership]),
      resolvePrincipal: async () => bound.principal })(request);
  } catch { return businessOutcomeReadNotConfiguredResponse(); } };
}
