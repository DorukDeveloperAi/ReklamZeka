import type { NodePgDatabase } from "drizzle-orm/node-postgres";
import { CategoryInventoryService } from "@/application/category-inventory-service";
import { DrizzleCategoryInventoryRepository } from "@/connectors/categories/category-inventory-drizzle-repository";
import * as schema from "@/db/schema";
import { categoryInventoryNotConfiguredResponse, categoryInventorySessionRequiredResponse,
  createCategoryInventoryHttpHandler } from "@/server/category-inventory-http";
import { LocalDecisionRoomBoundaryError, resolveTrustedLocalCategoryRegistryPrincipal,
  type LocalDecisionRoomConfig } from "@/server/local-decision-room-runtime";
import { LocalSessionCapabilityError } from "@/security/local-session-capability";

type Database = NodePgDatabase<typeof schema>;
export function createLocalCategoryInventoryHandler(input: Readonly<{
  database: Pick<Database, "execute">; config: LocalDecisionRoomConfig;
}>) {
  return async (request: Request) => { try {
    const bound = await resolveTrustedLocalCategoryRegistryPrincipal({ request, database: input.database,
      config: input.config });
    return createCategoryInventoryHttpHandler({ service: new CategoryInventoryService(
      new DrizzleCategoryInventoryRepository(input.database), [bound.membership]),
    resolvePrincipal: async () => bound.principal })(request);
  } catch (reason) {
    return reason instanceof LocalDecisionRoomBoundaryError || reason instanceof LocalSessionCapabilityError
      ? categoryInventorySessionRequiredResponse() : categoryInventoryNotConfiguredResponse();
  } };
}
