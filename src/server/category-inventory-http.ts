import { NextResponse } from "next/server";
import type { TrustedDecisionRoomPrincipal } from "@/application/decision-room-agent-contract";
import type { CategoryInventoryService } from "@/application/category-inventory-service";
import { CategoryInventoryRepositoryError } from "@/connectors/categories/category-inventory-drizzle-repository";
import { AuthorizationError } from "@/security/authorization";

const HEADERS = Object.freeze({ "Cache-Control": "private, no-store, max-age=0", "X-Content-Type-Options": "nosniff",
  "X-ReklamZeka-Access-Mode": "category-inventory-read", "X-ReklamZeka-Action-Authority": "none" });
const AUTHORITY = Object.freeze({ canDraft: false, canPublish: false, canArchive: false, canAssign: false,
  canWriteMeta: false, canAuthorizeAction: false, canEnforcePolicy: false });
function error(code: string, message: string, status: number) {
  return NextResponse.json({ error: { code, message }, authority: AUTHORITY }, { status, headers: HEADERS });
}
export function categoryInventoryNotConfiguredResponse() {
  return error("source_not_configured", "İç kategori envanteri yerel çalışma alanına henüz bağlanmadı.", 503);
}
export function categoryInventorySessionRequiredResponse() {
  return error("local_session_required", "İç kategori envanteri için yerel dashboard oturumunu bağlayın.", 401);
}
export function createCategoryInventoryHttpHandler(input: Readonly<{
  service: Pick<CategoryInventoryService, "list">;
  resolvePrincipal(request: Request): Promise<TrustedDecisionRoomPrincipal | null>;
}>) {
  return async (request: Request) => { try {
    const url = new URL(request.url);
    if (request.method !== "GET" || url.search || request.headers.has("authorization")
      || !request.headers.get("cookie") || request.headers.has("x-workspace-id") || request.headers.has("x-workspace-ref")
      || request.headers.get("sec-fetch-site") !== "same-origin"
      || request.headers.get("x-reklamzeka-intent") !== "category-inventory-read") {
      return error("invalid_input", "Kategori envanteri isteği geçersiz.", 400);
    }
    const principal = await input.resolvePrincipal(request); if (!principal) throw new AuthorizationError();
    return NextResponse.json(await input.service.list(principal), { headers: HEADERS });
  } catch (reason) {
    if (reason instanceof AuthorizationError) return error("forbidden", reason.publicMessage, 403);
    if (reason instanceof CategoryInventoryRepositoryError && reason.code === "workspace_scope_mismatch") {
      return error("forbidden", "Çalışma alanı erişilebilir değil.", 403);
    }
    return categoryInventoryNotConfiguredResponse();
  } };
}
