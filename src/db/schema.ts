...
export const guideRunActionBindings = pgTable("guide_run_action_bindings", {
  id: uuid("id").primaryKey().defaultRandom(), workspaceId: uuid("workspace_id").notNull().references(() => workspaces.id, { onDelete: "cascade" }), runId: uuid("run_id").notNull(), guideRevisionId: uuid("guide_revision_id").notNull(), dispositionArtifactId: uuid("disposition_artifact_id").notNull(), actionUnitId: uuid("action_unit_id").notNull(), proposalBundleId: uuid("proposal_bundle_id").notNull(), actionUnitRef: text("action_unit_ref").notNull(), actionUnitHash: text("action_unit_hash").notNull(), proposalRef: text("proposal_ref").notNull(), proposalHash: text("proposal_hash").notNull(), entityRef: text("entity_ref").notNull(), sliceRef: text("slice_ref").notNull(), marketKey: text("market_key").notNull(), effectiveGuideSetHash: text("effective_guide_set_hash").notNull(), resolutionHash: text("resolution_hash").notNull(), decision: text("decision").notNull().default("pending"), version: text("version").notNull().default("p06-action-binding/1.0.0"), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [uniqueIndex("guide_run_action_bindings_workspace_row_unique").on(table.workspaceId, table.id), uniqueIndex("guide_run_action_bindings_workspace_unit_unique").on(table.workspaceId, table.runId, table.actionUnitId), index("guide_run_action_bindings_workspace_revision_idx").on(table.workspaceId, table.guideRevisionId), index("guide_run_action_bindings_workspace_proposal_idx").on(table.workspaceId, table.proposalBundleId), index("guide_run_action_bindings_workspace_artifact_fk_idx").on(table.workspaceId, table.dispositionArtifactId), index("guide_run_action_bindings_workspace_unit_fk_idx").on(table.workspaceId, table.actionUnitId), index("guide_run_action_bindings_workspace_proposal_fk_idx").on(table.workspaceId, table.proposalBundleId), foreignKey({ columns: [table.workspaceId, table.runId, table.guideRevisionId], foreignColumns: [guideRuns.workspaceId, guideRuns.id, guideRuns.guideRevisionId], name: "guide_run_action_bindings_run_revision_fk" }).onDelete("cascade"), foreignKey({ columns: [table.workspaceId, table.dispositionArtifactId], foreignColumns: [guideRunArtifacts.workspaceId, guideRunArtifacts.id], name: "guide_run_action_bindings_disposition_fk" }).onDelete("restrict"), foreignKey({ columns: [table.workspaceId, table.actionUnitId], foreignColumns: [actionProposalUnits.workspaceId, actionProposalUnits.id], name: "guide_run_action_bindings_unit_fk" }).onDelete("restrict"), foreignKey({ columns: [table.workspaceId, table.proposalBundleId], foreignColumns: [actionProposalBundles.workspaceId, actionProposalBundles.id], name: "guide_run_action_bindings_proposal_fk" }).onDelete("restrict"), check("guide_run_action_bindings_contract", sql`${table.actionUnitRef} ~ '^action_unit_[a-f0-9]{20}$' and ${table.proposalRef} ~ '^action_bundle_[a-f0-9]{20}$' and ${table.actionUnitHash} ~ '^[a-f0-9]{64}$' and ${table.proposalHash} ~ '^[a-f0-9]{64}$' and ${table.entityRef} ~ '^[a-z][a-z0-9]{0,31}_[a-z0-9][a-z0-9_.:-]{0,126}$' and ${table.sliceRef} ~ '^slice_[a-z0-9][a-z0-9_.:-]{0,190}$' and ${table.marketKey} in ('yerli','yabanci') and ${table.effectiveGuideSetHash} ~ '^[a-f0-9]{64}$' and ${table.resolutionHash} ~ '^[a-f0-9]{64}$' and ${table.decision}='pending' and ${table.version}='p06-action-binding/1.0.0'`)]);
