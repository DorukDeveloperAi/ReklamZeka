...
}, (table) => [uniqueIndex("guide_runs_workspace_row_unique").on(table.workspaceId, table.id), uniqueIndex("guide_runs_workspace_revision_row_unique").on(table.workspaceId, table.id, table.guideRevisionId), uniqueIndex("guide_runs_workspace_ref_unique").on(table.workspaceId, table.runRef), uniqueIndex("guide_runs_workspace_idempotency_unique").on(table.workspaceId, table.idempotencyKey), index("guide_runs_workspace_revision_created_idx").on(table.workspaceId, table.guideRevisionId, table.createdAt), foreignKey({ columns: [table.workspaceId, table.guideId], foreignColumns: [guides.workspaceId, guides.id], name: "guide_runs_guide_scope_fk" }).onDelete("restrict"), foreignKey({ columns: [table.workspaceId, table.guideRevisionId, table.guideId], foreignColumns: [guideRevisions.workspaceId, guideRevisions.id, guideRevisions.guideId], name: "guide_runs_revision_scope_fk" }).onDelete("restrict"), check("guide_runs_contract", sql`${table.runRef} ~ '^guide_run_[a-f0-9]{24}$' and ${table.guideRevisionHash} ~ '^[a-f0-9]{64}$' and ${table.idempotencyKey} ~ '^guide_(slot|manual)_[a-f0-9]{64}$' and ${table.runVersion} = 'guide-run/1.2.0' and jsonb_typeof(${table.triggerPayload})='object' and octet_length(${table.triggerPayload}::text)<=16878`)]);
...
}, (table) => [uniqueIndex("guide_run_schedule_receipts_workspace_fire_unique").on(table.workspaceId, table.fireRef), uniqueIndex("guide_run_schedule_receipts_workspace_revision_slot_unique").on(table.workspaceId, table.guideRevisionId, table.scheduledFor), index("guide_run_schedule_receipts_workspace_revision_time_idx").on(table.workspaceId, table.guideRevisionId, table.scheduledFor), foreignKey({ columns: [table.workspaceId, table.guideRevisionId], foreignColumns: [guideRevisions.workspaceId, guideRevisions.id], name: "guide_run_schedule_receipts_revision_scope_fk" }).onDelete("restrict"), foreignKey({ columns: [table.workspaceId, table.runId, table.guideRevisionId], foreignColumns: [guideRuns.workspaceId, guideRuns.id, guideRuns.guideRevisionId], name: "guide_run_schedule_receipts_run_revision_scope_fk" }).onDelete("restrict"), check("guide_run_schedule_receipts_contract", sql`${table.fireRef} ~ '^guide_fire_[a-f0-9]{64}$' and ${table.receiptHash} ~ '^[a-f0-9]{64}$' and ${table.missedCount} between 0 and 1000000 and ((${table.missedFrom} is null and ${table.missedTo} is null and ${table.missedCount}=0) or (${table.missedFrom} is not null and ${table.missedTo} is not null and ${table.missedCount}>=1 and ${table.missedFrom} <= ${table.missedTo} and ${table.missedTo} <= ${table.scheduledFor}))`)]);
