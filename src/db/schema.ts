...
export const guides = pgTable("guides", {
  ...
  uniqueIndex("guides_workspace_row_unique").on(table.workspaceId, table.id), uniqueIndex("guides_workspace_ref_unique").on(table.workspaceId, table.guideRef), uniqueIndex("guides_workspace_identity_market_unique").on(table.workspaceId, table.id, table.guideRef, table.marketDefinitionId), uniqueIndex("guides_workspace_slice_market_unique").on(table.workspaceId, table.id, table.sliceId, table.marketDefinitionId), index("guides_workspace_live_idx").on(table.workspaceId, table.tombstonedAt, table.createdAt),
...
}, (table) => [uniqueIndex("guide_interpretation_acceptances_exact_unique").on(table.workspaceId, table.guideRevisionId, table.interpretationHash), index("guide_interpretation_acceptances_workspace_revision_idx").on(table.workspaceId, table.guideRevisionId), foreignKey({ columns: [table.workspaceId, table.guideRevisionId], foreignColumns: [guideRevisions.workspaceId, guideRevisions.id], name: "guide_interpretation_acceptances_revision_scope_fk" }).onDelete("restrict"), foreignKey({ columns: [table.workspaceId, table.acceptedByActorId], foreignColumns: [memberships.workspaceId, memberships.userId], name: "guide_interpretation_acceptances_actor_scope_fk" }).onDelete("restrict"), check("guide_interpretation_acceptances_hash", sql`${table.interpretationHash} ~ '^[a-f0-9]{64}$'`)]);
...
}, (table) => [uniqueIndex("guide_heads_workspace_guide_unique").on(table.workspaceId, table.guideId), foreignKey({ columns: [table.workspaceId, table.guideId], foreignColumns: [guides.workspaceId, guides.id], name: "guide_heads_guide_scope_fk" }).onDelete("cascade"), foreignKey({ columns: [table.workspaceId, table.latestRevisionId, table.guideId], foreignColumns: [guideRevisions.workspaceId, guideRevisions.id, guideRevisions.guideId], name: "guide_heads_latest_revision_scope_fk" }).onDelete("restrict"), foreignKey({ columns: [table.workspaceId, table.currentActiveRevisionId, table.guideId], foreignColumns: [guideRevisions.workspaceId, guideRevisions.id, guideRevisions.guideId], name: "guide_heads_active_revision_scope_fk" }).onDelete("restrict"), check("guide_heads_version_nonnegative", sql`${table.version} >= 0`)]);
...
}, (table) => [uniqueIndex("guide_lifecycle_events_workspace_key_unique").on(table.workspaceId, table.eventKey), index("guide_lifecycle_events_workspace_guide_idx").on(table.workspaceId, table.guideId, table.occurredAt), foreignKey({ columns: [table.workspaceId, table.guideId], foreignColumns: [guides.workspaceId, guides.id], name: "guide_lifecycle_events_guide_scope_fk" }).onDelete("restrict"), foreignKey({ columns: [table.workspaceId, table.guideRevisionId, table.guideId], foreignColumns: [guideRevisions.workspaceId, guideRevisions.id, guideRevisions.guideId], name: "guide_lifecycle_events_revision_scope_fk" }).onDelete("restrict"), foreignKey({ columns: [table.workspaceId, table.actorId], foreignColumns: [memberships.workspaceId, memberships.userId], name: "guide_lifecycle_events_actor_scope_fk" }).onDelete("restrict"), check("guide_lifecycle_events_type", sql`${table.eventType} in ('draft_created', 'interpretation_accepted', 'activated', 'superseded', 'paused', 'archived') and ${table.eventKey} ~ '^guide_event_[a-f0-9]{64}$'`)]);
...
}, (table) => [uniqueIndex("guide_activation_outbox_key_unique").on(table.workspaceId, table.activationKey), index("guide_activation_outbox_workspace_revision_idx").on(table.workspaceId, table.guideRevisionId, table.createdAt), foreignKey({ columns: [table.workspaceId, table.guideId], foreignColumns: [guides.workspaceId, guides.id], name: "guide_activation_outbox_guide_scope_fk" }).onDelete("restrict"), foreignKey({ columns: [table.workspaceId, table.guideRevisionId, table.guideId], foreignColumns: [guideRevisions.workspaceId, guideRevisions.id, guideRevisions.guideId], name: "guide_activation_outbox_revision_scope_fk" }).onDelete("restrict"), check("guide_activation_outbox_key", sql`${table.activationKey} ~ '^guide_activation_[a-f0-9]{64}$'`)]);
