...
export const findingLifecycleEvents = pgTable("finding_lifecycle_events", {
  ...
  namespace: text("namespace").notNull(), resolutionScope: text("resolution_scope").notNull(), fingerprint: text("fingerprint").notNull(), sequence: integer("sequence").notNull(), eventType: text("event_type").notNull(), state: text("state").notNull(), evidenceHash: text("evidence_hash").notNull(), previousEventHash: text("previous_event_hash").notNull(), eventHash: text("event_hash").notNull(), sourceOccurrenceHash: text("source_occurrence_hash").notNull(), reportHash: text("report_hash").notNull(), occurredAt: timestamp("occurred_at", { withTimezone: true }).notNull(), observationPayload: jsonb("observation_payload").$type<Record<string, unknown>>(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [
  uniqueIndex("finding_lifecycle_events_workspace_row_unique").on(table.workspaceId, table.id), uniqueIndex("finding_lifecycle_events_workspace_event_hash_unique").on(table.workspaceId, table.eventHash), uniqueIndex("finding_lifecycle_events_workspace_source_occurrence_unique").on(table.workspaceId, table.sourceOccurrenceHash), uniqueIndex("finding_lifecycle_events_workspace_fingerprint_sequence_unique").on(table.workspaceId, table.namespace, table.resolutionScope, table.fingerprint, table.sequence), index("finding_lifecycle_events_workspace_scope_time_idx").on(table.workspaceId, table.namespace, table.resolutionScope, table.occurredAt),
  check("finding_lifecycle_events_contract", sql`${table.namespace} ~ '^[a-z][a-z0-9_.:-]{0,63}$' and octet_length(${table.resolutionScope}) between 1 and 256 and ${table.fingerprint} ~ '^[a-z][a-z0-9_.:-]{0,127}$' and ${table.sequence} between 1 and 1000000 and ${table.eventType} in ('opened','observed','resolved','reopened') and ${table.state} in ('open','resolved') and ${table.evidenceHash} ~ '^[a-f0-9]{64}$' and ${table.previousEventHash} ~ '^(0{64}|[a-f0-9]{64})$' and ${table.eventHash} ~ '^[a-f0-9]{64}$' and ${table.sourceOccurrenceHash} ~ '^[a-f0-9]{64}$' and ${table.reportHash} ~ '^[a-f0-9]{64}$' and (${table.observationPayload} is null or (jsonb_typeof(${table.observationPayload}) = 'object' and octet_length(${table.observationPayload}::text) <= 16384))`),
]);
export const findingHeads = pgTable("finding_heads", {
  id: uuid("id").primaryKey().defaultRandom(), workspaceId: uuid("workspace_id").notNull().references(() => workspaces.id, { onDelete: "cascade" }), namespace: text("namespace").notNull(), resolutionScope: text("resolution_scope").notNull(), fingerprint: text("fingerprint").notNull(), sequence: integer("sequence").notNull(), state: text("state").notNull(), evidenceHash: text("evidence_hash").notNull(), eventHash: text("event_hash").notNull(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [uniqueIndex("finding_heads_workspace_row_unique").on(table.workspaceId, table.id), uniqueIndex("finding_heads_workspace_fingerprint_unique").on(table.workspaceId, table.namespace, table.resolutionScope, table.fingerprint), index("finding_heads_workspace_scope_state_idx").on(table.workspaceId, table.namespace, table.resolutionScope, table.state), index("finding_heads_workspace_event_fk_idx").on(table.workspaceId, table.eventHash), foreignKey({ columns: [table.workspaceId, table.eventHash], foreignColumns: [findingLifecycleEvents.workspaceId, findingLifecycleEvents.eventHash], name: "finding_heads_event_scope_fk" }).onDelete("restrict"), check("finding_heads_contract", sql`${table.namespace} ~ '^[a-z][a-z0-9_.:-]{0,63}$' and octet_length(${table.resolutionScope}) between 1 and 256 and ${table.fingerprint} ~ '^[a-z][a-z0-9_.:-]{0,127}$' and ${table.sequence} between 1 and 1000000 and ${table.state} in ('open','resolved') and ${table.evidenceHash} ~ '^[a-f0-9]{64}$' and ${table.eventHash} ~ '^[a-f0-9]{64}$'`)]);

/** Proposed-only Development Log records; no system or agent transition exists. */
