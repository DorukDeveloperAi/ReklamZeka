...
| R-G4 | Metrik, bulgu, plan ve eylem snapshot + context/tanım/guidance/practice/policy sürüm bağını korur. | trace contract suite |
...
| R-G10 | Aynı snapshot + tanım/policy sürümleri deterministic metrik/finding/eligibility/budget çıktısını byte-eş üretir; agent sentezi ayrı sürümlü/auditli çıktıdır. | deterministic replay boundary |
...
| R-G13 | Yeni kategori, mapping, guidance, advised practice, kural, playbook ve schedule kod değişikliği olmadan tanım verisiyle eklenir. | definition-only proof |
...
| R-G16 | Düz metin sürümlü/scoped guidance olarak analizde kullanılabilir ama doğrudan hard constraint/action authorization olamaz; enforceable kullanım typed promotion, etki preview ve yayın ister. | guidance-to-policy E2E |
| R-G17 | Core sync/classification/metric/feature/finding/policy/budget/action motorları model/provider SDK'sı import etmez; modelsiz deterministik koşum tamdır. Agentic yorum/sentez ayrı ve opsiyoneldir. | import-boundary + no-model E2E |
...
| R-G23 | Agentic analitik guidance esnek olabilir; metrik/formül, tenant, policy precedence, bütçe uzlaşması, approval ve action valve deterministik sınır olarak kalır. | soft-vs-enforceable boundary suite |
| R-G24 | Sistem activity üretmek için öneri üretmez; settle/learning/cooldown/observation ve tekrar sınırlarıyla `no-change` geçerli karardır. | decision-tempo/hyperactivity suite |
| R-G25 | İlk mimari modular monolith+PostgreSQL+DB worker'dır; vector DB/warehouse/event bus/microservice yalnız ölçülen ölçek eşiği ve ADR ile eklenir. | dependency/architecture budget gate |
| R-G26 | Agentic sohbet sessiz kural öğrenemez; AdvisedPractice→standardization→policy geçişleri versioned, outcome-backed ve açık kullanıcı kararıdır. | silent-learning/promotion negatives |
...
| R-08.13 | L0 raw payload source/hash/version ve configurable retention taşır; agent context'ine doğrudan girmez, disconnect/revoke/export/delete yaşam döngüsü auditlidir. | raw-retention/disconnect/privacy suite |
...
| R-09.15 | GuidanceSource raw kullanıcı sözü, official Meta source, business strategy, observed result, experiment outcome ve operating note provenance türlerini ayrı saklar. | guidance source/provenance suite |
| R-09.16 | GuidanceCard strict DSL olmadan natural-language body, must/should/consider/avoid/question derecesi, topic, scope facets, source refs, lifecycle ve version taşır. | flexible guidance schema/E2E |
| R-09.17 | Guidance global/account-group/account/objective/funnel/internal-category/lifecycle/entity/promotion-template/topic boyutlarına çoklu bağlanabilir. | guidance binding matrix |
| R-09.18 | G0 raw→G1 scoped→G2 reviewed→G3 typed policy→G4 automation olgunluğunda G2→G3 semantic diff, historical replay, conflict/impact preview ve kullanıcı onayı ister. | progressive formalization E2E |
| R-09.19 | Official Meta best-practice card source ref, captured/review-by tarih, applicable scope ve freshness olmadan “official” yayınlanamaz; eski sürüm tarihsel run'ı değiştirmez. | best-practice provenance/freshness suite |
| R-09.20 | AdvisedPractice problem/scope/inputs/steps/rationale/cadence/exceptions/evidence/confidence ve candidate→trial→outcome→standardization lifecycle taşır. | advised-practice lifecycle suite |
| R-09.21 | StandardizationReview practice'i deterministic feature, agenda, playbook, cadence, guidance, policy ve human-judgment parçalarına ayırabilir; tamamını rule yapma zorunluluğu yoktur. | practice decomposition/review E2E |
...
| R-10.9 | AnalysisAgenda genel→group/account→objective→internal-category→entity→topic→history sıralı pass'lerini versioned taşır; kullanıcı başlık/category alt kümesi seçebilir. | agenda order/subset replay |
| R-10.10 | EffectiveGuidancePack applied/suppressed/conflicting/missing card'ları source/scope/relevance reason ve freshness ile run'a snapshot'lar. | guidance context snapshot |
| R-10.11 | DecisionCadenceProfile settle delay, observation, learning/cooldown, max decision/action frequency, repeat cooldown, evidence threshold ve emergency exception taşır. | cadence golden/negative matrix |
| R-10.12 | ExperimentRecord hypothesis/baseline/primary metric/guardrail/min sample/window/stop condition ve winner/loser/inconclusive outcome taşır; contamination görünürdür. | experiment lifecycle/replay suite |
| R-10.13 | L0→L5 pipeline raw/canonical/features/windows/evidence/compact-context katmanlarını source/formula/catalog versions ile kurar; L2–L5 incremental invalidate/rebuild olur. | processing/replay/invalidation suite |
| R-10.14 | EffectiveCampaignContext Meta config, categories, guidance, enforceable policies, cadence, budget, data quality/features ve decision/practice/outcome history'yi frozen hash ile taşır. | effective-context golden/replay |
| R-10.15 | Analiz top-down agenda ve bounded bottom-up driver path kullanır; campaign finding driver veya unresolved reason taşır. | bidirectional-driver suite |
| R-10.16 | Optional BusinessOutcomeSignal manual/CSV ile qualified lead/appointment/sale/revenue/invalid lead taşır; eksik mapping Meta proxy'yi gerçek sonuç yapmaz. | outcome-signal provenance/mapping suite |
...
| id | requirement | doğrulama |
|---|---|---|
| R-12.1 | Prompt envelope sabit safety + context/playbook + EffectiveGuidancePack + enforceable policies + findings + budget simulation + user guidance verisidir. | envelope snapshot |
...
| R-12.3 | Kanıt/öneri ifadeleri findingId/policyId/guidanceCardId/sourceRef/experimentId/simulationId türlerinden uygun citation taşır; yeni metrik veya eylem uyduramaz. | claim/citation validator |
| R-12.4 | Doğal dil önce scoped GuidanceCard/Set taslağı olabilir; yalnız enforceable clause strict policy promotion'a girer, bilinmeyen scope/kaynak/çatışma kullanıcıya sorulur. | NL-guidance-to-policy fixture/eval |
...
| R-12.6 | Client/session, envelope/schema/sanitization sürümü, context/input hash, output/citations, redaksiyon ve durum audit edilir; model/token/usage yalnız yerel istemci güvenilir biçimde sağlarsa opsiyonel telemetridir. | advisor ledger suite |
| R-12.7 | Local agent yok/hata durumunda deterministic metric/finding/budget/eligibility ve scheduled baseline sürer; conversational synthesis unavailable sebebi kaydedilir. | fallback tests |
...
| R-12.14 | Agent guidance oluştururken kullanıcı yaklaşımı, ilgili mevcut talimatlar, kaynaklı Meta best-practice, kanıt/experiment ve conflictleri kritik interview akışında birlikte gösterir. | deliberation journey/eval |
| R-12.15 | Guidance retrieval önce deterministic scope filter, sonra bounded relevance ranking kullanır; semantic ranking enforceable policy/action authorization seçemez. | retrieval boundary negatives |
| R-12.16 | Agent her karar turunda act/test/observe/no-change seçeneklerini değerlendirir ve cadence ihlalinde proposal'ı bastırır veya review-only işaretler. | no-change/cadence eval suite |
...
| R-13.9 | Run ledger resolved window/snapshot/context-definition-policy versions/status/attempt/error/artifacts taşır; provider cost zorunlu alan değildir. | run ledger contract |
...
| R-13.12 | Manual/assisted/automated-read/scheduled-plan planlama modları ile approval-only/policy-limited execution profilleri ayrı versioned scope policy'leridir; child daha riskliye genişleyemez. | hybrid-mode inheritance suite |
| R-13.13 | Policy-limited yalnız K1 ve explicit izinli/cap'li K2; K3/K4 tüm CLI/agent/schedule yollarında insan onaylıdır. | autonomy bypass negative matrix |
...
| R-14.11 | Agent console Codex CLI/VS Code, Claude Code ve ek yerel MCP istemci oturumlarını tek trace'te ayırır; istemci değiştirme proposal semantics'ini değiştirmez ve ReklamZeka provider API çağırmaz. | local-client-switch golden E2E |
...
| R-14.18 | Guidance studio kritik chat + canlı card paneli, raw user statement/agent synthesis/source, scope binding, applied-in-analysis, conflict ve promote-to-policy akışını sade gösterir. | guidance studio browser/a11y E2E |
